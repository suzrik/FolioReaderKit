...
internal let kSepiaMode = "com.folioreader.kSepiaMode"
...
@objc public protocol FolioReaderDelegate: class {
    ...
    @objc optional func folioReader(_ folioReader: FolioReader, jumpToSyncAudio ms: Double)

    @objc optional func folioReader(_ folioReader: FolioReader, didFinishedLoadingPage page: FolioReaderPage)
...
extension FolioReader {
        ...
        set (value) {
            ...
            if let readerCenter = self.readerCenter {
                UIView.animate(withDuration: 0.6, animations: {
                    ...
                    readerCenter.collectionView.backgroundColor = (self.nightMode == true ? self.readerContainer?.readerConfig.nightModeBackground : UIColor(red: 226/255, green: 226/255, blue: 226/255, alpha: 1.0))
    ...
    open var sepiaMode: Bool {
        get { return self.defaults.bool(forKey: kSepiaMode) }
        set (value) {
            self.defaults.set(value, forKey: kSepiaMode)

            if let readerCenter = self.readerCenter {
                UIView.animate(withDuration: 0.6, animations: {
                    _ = readerCenter.currentPage?.webView?.js("sepiaMode(\(self.sepiaMode))")
                }, completion: { (finished: Bool) in
                    NotificationCenter.default.post(name: Notification.Name(rawValue: "needRefreshPageMode"), object: nil)
                })
            }
