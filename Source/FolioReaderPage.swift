...
open class FolioReaderPage: UICollectionViewCell, WKNavigationDelegate, WKUIDelegate, UIGestureRecognizerDelegate {
    ...
    open var colorView: UIView = UIView()
    ...
    public func setup(withReaderContainer readerContainer: FolioReaderContainer) {
        ...
        if webView == nil {
            ...
            if #available(iOS 10.0, *) {
                webViewCofig.dataDetectorTypes = [.link]
            } else {
                // Fallback on earlier versions
            }

            webView = FolioReaderWebView(frame: webViewFrame(), readerContainer: readerContainer, configuration: nil)
    ...
    //open func webView(_ webView: WKWebView, shouldStartLoadWith request: URLRequest, navigationType: WKNavigationType) -> Bool {
        ...
        if scheme == "highlight" || scheme == "highlight-with-note" {
            ...
            decisionHandler(.cancel)
            return
        } else if scheme == "jump-audio" {
            guard let decoded = url.absoluteString.removingPercentEncoding else {
                decisionHandler(.cancel)
                return
            }
            let index = decoded.index(decoded.startIndex, offsetBy: 13)
            let ms = Double(decoded[index...])
            self.folioReader.delegate?.folioReader?(self.folioReader, jumpToSyncAudio: ms ?? 0)
    ...
    @objc open func handleTapGesture(_ recognizer: UITapGestureRecognizer) {
        ...
        if let _navigationController = self.folioReader.readerCenter?.navigationController, (_navigationController.isNavigationBarHidden == true) {
            ...
            webView?.js("getSelectedText()", completionHandler: { (selected, _) in
                ...
                DispatchQueue.main.asyncAfter(deadline: dispatchTime, execute: {
                    if (self.shouldShowBar == true && self.menuIsVisible == false) {
                        self.folioReader.readerCenter?.toggleBars()//suzrik
                    }
                })
    ...
    override open func canPerformAction(_ action: Selector, withSender sender: Any?) -> Bool {
        ...
        if !webView.isShare && !webView.isColors {
            webView.js("getSelectedText()") { (result, _) in
                if result != nil && result!.components(separatedBy: " ").count == 1 {
    ...
    @objc func refreshPageMode() {
        ...
        var contentHeight:String = "0";
        webView.evaluateJavaScript(script) { (result, error) in
            contentHeight = result as? String ?? "0"
        }
        let frameHeight = webView.frame.height
        let pageCount = ceil(CGFloat(Double(contentHeight)!) / frameHeight)
        let lastPageHeight = frameHeight * CGFloat(pageCount) - CGFloat(Double(contentHeight)!)
        colorView.frame = CGRect(x: webView.frame.width * pageCount, y: webView.frame.height - lastPageHeight, width: webView.frame.width, height: lastPageHeight)
    }
