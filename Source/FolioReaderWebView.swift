 ...
 */
    ...
    init(frame: CGRect, readerContainer: FolioReaderContainer, configuration: WKWebViewConfiguration?) {
    ...
    @objc func share(_ sender: UIMenuController) {
        ...
        let shareImage = UIAlertAction(title: self.readerConfig.localizedShareImageQuote, style: .default, handler: { (action) -> Void in
            if self.isShare {
                ...
                self.js("getHighlightContent()") { (textToShare, _) in
                ...
                self.js("getSelectedText()") { (textToShare, _) in
                    if let textToShare = textToShare {
                        self.folioReader.readerCenter?.presentQuoteShare(textToShare)
    ...
    func remove(_ sender: UIMenuController?) {
        js("removeThisHighlight()") { (removedId, _) in
    ...
    @objc func highlight(_ sender: UIMenuController?) {
        ...
        js("highlightString('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { (highlightAndReturn, _) in
    ...
    @objc func highlightWithNote(_ sender: UIMenuController?) {
        ...
        js("highlightStringWithNote('\(HighlightStyle.classForStyle(self.folioReader.currentHighlightStyle))')") { (highlightAndReturn, _) in
    ...
    @objc func updateHighlightNote (_ sender: UIMenuController?) {
        js("getHighlightId()") { (highlightId, _) in
    ...
    @objc func define(_ sender: UIMenuController?) {
        js("getSelectedText()") { (selectedText, _) in
    ...
    func changeHighlightStyle(_ sender: UIMenuController?, style: HighlightStyle) {
        ...
        js("setHighlightStyle('\(HighlightStyle.classForStyle(style.rawValue))')") { (updateId, err) in
    ...
    open func js(_ script: String, completionHandler: ((String?, Error?) -> Void)? = nil) {
        //let callback = self.stringByEvaluatingJavaScript(from: script)
        //var callback:String?
        
        print("js call: \(script)")
