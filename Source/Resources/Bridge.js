...
//      var allWords = document.getElementsByClassName('word-sync');
//      for (var i = 0; i < allWords.length; i++) {
//        allWords[i].addEventListener("click", function(event) {
//         if (isVerticalScroll) {
//             var index = this.id.replace('w', '') * 1 + 1; // находим "индекс слова"
//             if (index >= aData.length) {
//               index = aData.length - 1;
//             }
//
//             var highlightAudioPositon = aData[index];
//             var playAudioPositon = aData[index-1];
//             selectWordByTimeFixed(highlightAudioPositon);
//             var URLBase = "jump-audio://";
//             window.location = URLBase + encodeURIComponent(playAudioPositon)
//         }
//        })
//      }
//});
...
// Toggle sepia mode
function sepiaMode(enable) {
    var elm = document.documentElement;
    if(enable) {
        addClass(elm, "sepiaMode");
    } else {
...
function setScrollDirection(isVerticalDirection) {
    isVerticalScroll = isVerticalDirection
}
...
//audio sync
function selectWordByTimeFixed(time) {
    var wordIndex = searchWordIndex(time);
    ...
    var word = document.getElementById("w" + wordIndex);
    word.className += ' word-select';

    document.body.myIsDoScroll = false
    if(word.offsetTop > window.innerHeight/2 && !document.body.myIsDoScroll && window.pageYOffset != Math.round(word.offsetTop - window.innerHeight/2)) {
        document.body.myScrollTo = Math.round(word.offsetTop - window.innerHeight*0.4);
        ...
        slowScrollFixed();
    } else if(!document.body.myIsDoScroll && window.pageYOffset > 0 && word.offsetTop <= window.innerHeight*0.4) window.scrollTo(0, 0);
}

function slowScrollFixed() {
    if (Math.abs(document.body.myScrollTo - window.pageYOffset) > Math.abs(document.body.myStep)) {
        window.scrollTo(0, window.pageYOffset + document.body.myStep);
        setTimeout(slowScrollFixed, 20);
    } else {
        window.scrollTo(0, document.body.myScrollTo);
        document.body.myIsDoScroll = false;
    }
}

function clearWordHighlighting() {
    var wordSelectedList = document.getElementsByClassName('word-select');
    for (var i = 0; i < wordSelectedList.length; i++) {
        wordSelectedList[i].className = 'word-sync';
    }
}

function searchWordIndex(a){
    var n = binarySearch(aData, a, compare_number);
    return n;
}
